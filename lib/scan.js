...
function aggregate(records, range, pricing, sessionMeta, opts, marks) {
  ...
  return {
    ...
    compact: compactSuggestions(sel, pricing, meta, opts, now, marks),
...
/**
 * Marks: "I compacted this session at time T". Everything before T is ignored,
 * so the suggestion starts counting again from the click. Stored as
 * { sessionId: epochMs } in compact-marks.json next to server.js.
 */
const MARKS_FILE = path.join(ROOT, 'compact-marks.json');

function loadMarks() {
  try {
    const m = JSON.parse(fs.readFileSync(MARKS_FILE, 'utf8'));
    const out = {};
    for (const k in m) if (Number.isFinite(m[k])) out[k] = m[k];
    return out;
  } catch {
    return {};
  }
...
function compactSuggestions(sel, pricing, sessionMeta, opts, now, marks) {
